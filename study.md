...
```md
Object-relational mapping is a way of translating data beween databases and object-oriented programming languages
```
...
```md
create, this does cause the created objects to persist in the database
```
...
```ruby
Movie.where(common_name: 'Red Panda')
Movie.where(phylum: 'Mollusca')
Movie.last
```
...
```ruby
galaxy = Galaxy.find_by(designation: 'Andromeda')
