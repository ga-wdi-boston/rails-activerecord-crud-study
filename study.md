...
```md
ORM is a way to convert data between two systems that require different data models.

Using an ORM is valuable to pass data between applications or systems with each system requiring the data to be storied and transacted in different ways. For example, between an application and a database.
```
...
```md
Create, yes it persists objects into the database.
```
...
organism = Organism.where(phylum: "Mollusca").last
```
