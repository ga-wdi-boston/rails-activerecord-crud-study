...
```md
ORM or object relational mapping allows the properties and relationships of objects
in an application to be stored and accessed easily.
```
...
```ruby
pandas = Organism.where(common_name: "Red Panda")
mollusks = Organism.where(phylum: "Mollusca").last
```
