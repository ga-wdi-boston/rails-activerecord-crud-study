...
```md
ORM allows programmers to store and retrieve objects without having to directly write SQL to access it.
```
...
```md
create and new both create new objects.  but the create method will also save it to the database
```
...
```ruby
g = Galaxy.find_by(designation: 'NGC 224')
g.update(name: 'Andromdea')
```
