...
```md
ORM = Object Relational Mapping.  It connects objects of an application to tables in a
relational database.  Using ORM you can store and retrieve properties and relationships
of the objects with writing SQL statements directly.  This also requires less overall
database code.
```
...
```md
new (will not save it to the DB unless 'save' is subsequently called)
create (will save to the DB)
```
...
mollusca_organisms = Organism.where(phylum: 'Mollusca')
last = Organism.last
```
