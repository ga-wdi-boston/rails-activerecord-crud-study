...
```md
it is the objects that are represented in the file
```
...
```md
CREATE, it will persist objects into rows
...
```ruby
not sure what you ar asking, there are more than one movie.
```
...
```ruby
Red Panda = organisms.find_by(name: 'Red Panda')
```
